...
void setup() {
  ...
  */
  ...
  CanControl.filters.setRB1(MASK_Sxxx, MC_VELOCITY_ID, MC_PHASE_ID, DC_INFO_ID, 0);
...
inline void defaultdisplay() {
  ...
  switch (steering_wheel.displayNumber) {
    ...
    case 1: // Motor Current
      ...
      screen.print("M:");
      screen.setCursor(2, V);
      screen.print(steering_wheel.MCurrentdisplay,1);
      break;
    case 2: // Array Current
      screen.setCursor(2, V - 2);
      screen.print("A:");
      screen.setCursor(2, V);
      screen.print(steering_wheel.ACurrentdisplay,1);
      break;
    case 3: // Battery Current
      screen.setCursor(2, V - 2);
      screen.print("B:");
      screen.setCursor(2, V);
      screen.print(steering_wheel.BCurrentdisplay,1);
      break;
    case 4: // Pack Voltage
      screen.setCursor(2, V - 2);
      screen.print("P:");
...
void loop() {
  ...
  if (CanControl.Available()) {
    ...
    switch (f.id)
    {
      ...
      case MC_PHASE_ID:
        {
          MC_PhaseCurrent packet(f); // Get Motor Current
          steering_wheel.MCurrentdisplay = (packet.phase_a + packet.phase_b)/2;
          CAN_RX.reset();
          break;
        }
      case DC_TEMP_0_ID:
